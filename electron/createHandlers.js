/**
 * Handlers IPC/HTTP compartilhados entre Electron e API web (Vercel).
 */
...
/**
 * @param {{ getMainWindow?: () => any, openExternal?: (url: string) => Promise<any> }} deps
 */
function createHandlers(deps = {}) {
  const getMainWindow = deps.getMainWindow || (() => null);
  const openExternal = deps.openExternal || (async (url) => {
    // Web: caller opens URL in browser
  ...
  const handlers = {
    ...
    'auth:restore': (_, userId, token) => auth.restoreSession(userId, token),
    ...
    'produtos:etiqueta': async (_, data) => {
      ...
      return salvarEAbrirPdf(getMainWindow(), {
