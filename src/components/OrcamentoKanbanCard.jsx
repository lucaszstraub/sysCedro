...
import { MOTIVO_ENCERRAMENTO_LABEL } from '../constants/orcamento';
import { formatCurrency, formatDate } from '../utils/format';

export default function OrcamentoKanbanCard({
  orcamento,
  onDragStart,
  onDragOver,
  onDrop,
    ...
    >
      ...
      <div className="kanban-card-meta">
        ...
        {Number(orcamento.desconto) > 0.005 && (
          <span className="hint-text" title="Desconto extra sobre o subtotal dos itens">
            Desc. extra {formatCurrency(orcamento.desconto)}
          </span>
        )}
        <span className="hint-text">{orcamento.total_itens || 0} item(ns)</span>
      </div>
      ...
      <div className="kanban-card-actions">
        <Link
          to={`/ferramentas-venda/orcamentos/${orcamento.id}`}
        ...
        {orcamento.status === 'aprovado' && (
          orcamento.venda_numero ? (
            <Link
              to={`/ferramentas-venda/vendas/${orcamento.venda_id}/editar`}
              className="btn btn-primary btn-sm"
              onClick={(e) => e.stopPropagation()}
              title="Venda vinculada a este orçamento"
            >
              {orcamento.venda_numero}
            </Link>
          ) : (
