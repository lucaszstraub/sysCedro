...
import { api } from '../api';
import { STATUS_LABEL } from '../constants/orcamento';
import { formatCurrency, formatDate } from '../utils/format';

export default function SelecionarOrcamentoModal({ onClose, onSelect, orcamentoAtualId }) {
  ...
  const handleSelect = async (orcamento) => {
    try {
      const completo = await api.getOrcamento(orcamento.id);
      onSelect(completo);
      onClose();
    } catch (err) {
      setError(err.message);
      ...
      <div className="modal modal-xl picker-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            ...
            <p className="picker-subtitle">Selecione um orçamento para preencher os dados da venda</p>
          </div>
          ...
          <div className="picker-filters">
            <div className="picker-search-wrap">
              <input
                ...
                placeholder="Pesquisar por número, cliente ou status..."
                value={busca}
