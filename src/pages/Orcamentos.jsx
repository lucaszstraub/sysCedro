...
import {
  KANBAN_COLUMNS,
  resolverColunaKanban,
} from '../constants/orcamento';
import { useAuth } from '../context/AuthContext';
import { useFeedback } from '../context/FeedbackContext';
import PageAlert from '../components/PageAlert';
import OrcamentoKanbanCard from '../components/OrcamentoKanbanCard';
...
export default function Orcamentos() {
  ...
  const load = async (term = busca) => {
    ...
    setError('');
    try {
      const lista = await api.listOrcamentos(term);
      setOrcamentos(lista);
    } catch (err) {
  ...
  const moverParaColuna = async (orcamentoId, colunaId) => {
    ...
    try {
      await api.moverOrcamentoKanban(orc.id, payload);
  ...
  const handleIniciarVenda = (orcamento) => {
    navigate(`/ferramentas-venda/vendas/novo?orcamento=${orcamento.id}`);
  };
  ...
  return (
    <>
      <header className="page-header">
        <h2>Orçamentos</h2>
        <p>Quadro kanban — orçamentos vencidos vão automaticamente para Rejeitado / Expirado</p>
      </header>
      ...
      <div className="toolbar">
        ...
        <Link to="/ferramentas-venda/orcamentos/novo" className="btn btn-primary">
          + Novo orçamento
        </Link>
      </div>
            ...
            >
              ...
              {coluna.id === 'aprovado' && (
                <p className="hint-text kanban-column-hint">
                  Orçamentos aprovados podem iniciar a venda. Após salvar a venda, o número dela aparece no card.
                </p>
              )}
              {coluna.id === 'encerrado' && (
                <p className="hint-text kanban-column-hint">
